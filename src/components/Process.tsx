...
export default function Process() {
  return (
    <section className="py-24 px-8 lg:px-20 relative z-10 bg-slate-50 dark:bg-complementary-950 border-y border-gray-200 dark:border-gray-900" id="process">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-2 gap-8">
          {processSteps.map((item, idx) => (
            <motion.div
              key={idx}
              className="group relative p-6 bg-white dark:bg-complementary-900/30 border border-gray-200 dark:border-gray-800/60 rounded-2xl hover:border-primary-500/30 hover:shadow-[0_10px_35px_-10px_rgba(88,117,245,0.12)] transition-all duration-300 backdrop-blur-sm"
            ...
            >
              {/* Step indicator */}
              <div className="absolute top-4 right-6 text-4xl font-extrabold text-gray-300 dark:text-gray-800/20 select-none group-hover:text-primary-500/20 transition-colors duration-300">
                {item.step}
              </div>
              ...
              {/* Title & Description */}
              <h3 className="text-lg md:text-xl lg:text-xl font-bold text-gray-900 dark:text-white mb-3 font-syne transition-colors duration-300 group-hover:text-primary-600 dark:group-hover:text-primary-300">{item.title}</h3>
